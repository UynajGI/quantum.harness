...
- **Output** — which figure(s)/panel(s) (one computation can feed several); and **per figure**: the observable plotted (y-axis) + normalization + which states it uses; the x-axis (parameter swept, range, spacing); what we expect to see and what would count as reproduced. When the figures share one computation these per-figure facts are paper-stated confirmations, not forks — present them as **one reproduction-targets table**, a row per figure with plain-English cells (what it shows / x-axis & range / what counts as reproduced), ratified in a single `Confirm / Show source / Override` turn like the setup card. Spend question turns only on genuine forks (scope, cost, deviations). **When the scope options carry materially different costs** (e.g. one panel runs at small size on a laptop while another needs a large lattice / GPU / cluster), the figure-scope question *is* a cost decision: give each option an order-of-magnitude **wall-time band pulled forward from the `/method-*` cost model** — the firm work count (flips / sweeps / matvecs) ÷ a stated assumed rate, with the rate named as the pre-probe unknown — so the choice is never cost-blind. This means pre-reading the likely method card's cost section during step 0; the method choice itself is still confirmed at step 1. Never present scope options whose cost spans orders of magnitude with only vague hints ("laptop-feasible", "wants a cluster") and no numbers. The detailed per-run-point cost table still comes in step 4; this is only the coarse band needed to choose scope.
...
3. **`/method-*` skill** — `/method-ed`, `/method-mps`, `/method-peps`, `/method-qmc`, `/method-ltrg`, `/method-vmc`, `/method-qcs`, `/method-mf`, `/method-polyopt`. Source for **step 1** (which method suits the target) and **step 2** (which tool the method routes to), plus **step 3 method setup** (the conceptual knobs and tricks).
